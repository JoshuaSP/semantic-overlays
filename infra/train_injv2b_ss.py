"""Modal app: injv2b recipe + the SHORT-SPAN increment (injv2b-ss).

Identical to train_injv2b_6x_per4.py in every hyperparameter -- same adapter
shape, LR, schedule, epochs, per-unit fanout. The ONLY difference is the
dataset: injv2b-ss = injv2b plus 1,172 fixed short-span items
(notes/short_span_corpus_plan.md), which fill the corner the shipped adapter
never trained on (spans under ~216 chars, payload-dominant, bare-framed) and
where the AlpacaFarm/StruQ leak lives (Qwen 14.4% begin-with; Llama 0.0%).

Held constant deliberately so any change in the eval sweep is attributable to
the data increment and nothing else. The GCG adversarial-training arm is kept
OUT of this run for the same reason (Joshua, 2026-08-27): it is likely to need
several iterations, and confounding it with this would make neither result
readable.
...
Objective (Joshua, 2026-08-10): CE toward the frozen model's own completion on
...
Architecture: per-layer SwiGLU bottleneck, d_hidden=128 (Joshua's call — skip
...
N_GPU = 8
D_HIDDEN = 128               # Joshua, 2026-08-10: MLP 128, no layervec arm
LOCAL_ACCUM = 4
EFF_BATCH = N_GPU * LOCAL_ACCUM
EPOCHS = 2                   # Joshua, 2026-08-11: 3 epochs likely overkill at 4x data
...
app = modal.App("inference-goggles-inject-ss")
...
def _micro(model, goggles, ctx, i, dev, need_grad, step=0):
    ...
    ALL arms are CE (Joshua, 2026-08-12). KL-for-both cost 14 SEP points; the
...
def main(run_name: str = "injv2b-ss-6x-per4", lr: float = LR, epochs: int = EPOCHS,
         wandb_entity: str = "vectorsmith", smoke_only: bool = False,
         warmup: int = WARMUP):
    if smoke_only:
        smoke.remote()
    else:
